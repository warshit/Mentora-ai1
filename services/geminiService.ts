...
export const getGeminiResponse = async (
  ...
  try {
    ...
    return { text: "Neural connection lost (RPC Error). Please check your internet connection and try again.", doubtType: 'General' as DoubtType, mode: LearningMode.UNDETERMINED };
  }
};
...
export const generateLearningInsights = async (student: StudentProfile): Promise<string[]> => {
  try {
    ...
    const response = await withRetry<GenerateContentResponse>(() => ai.models.generateContent({
      model: 'gemini-3-pro-preview', // Flash is sufficient and fast for insights
      contents: prompt,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
