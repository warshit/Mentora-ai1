...
import InactivityDebugger from './components/InactivityDebugger';
...
import { getAIHistory } from './services/firebaseService';
...
import { safeLocalStorageGet, safeLocalStorageSet, handleApiError } from './utils/errorHandling';
import { useInactivityTimer } from './hooks/useInactivityTimer';

// Safe Storage Helper - Now using utility functions
...
const AppContent: React.FC = () => {
  ...
  const [student, setStudent] = useState<StudentProfile | null>(() => {
    // Initialize from Firebase user data if available
    if (userData) {
      return {
        rollNumber: userData.rollNumber,
        name: userData.rollNumber, // Use roll number as name for now
        email: userData.email,
        year: 'III', // Default values - can be enhanced later
        department: 'CSE',
        memory: { historicalWeakTopics: [], completedCredits: 0 },
        topicNotes: {},
        skillHistory: [],
        preferences: {
          theme: 'light', 
  ...
  // Define handleLogout after student state is declared
  const handleLogout = useCallback(async (skipConfirmation = false) => {
    ...
    try {
      ...
      // We'll create a version with showToast later
    }
  }, []);

  // Create handleLogout with toast after showToast is available
  const handleLogoutWithToast = useCallback(async (skipConfirmation = false) => {
    if (!skipConfirmation && !confirm("Are you sure you want to logout?")) {
      return;
    }
  ...
  // Auto-logout with inactivity detection (30 seconds for testing)
  // Only activate when user is logged in
  const { showWarning, timeLeft, resetTimer, isPaused } = useInactivityTimer({
    timeout: 30000, // 30 seconds for testing (easier to see)
    warningTime: 10000, // 10 seconds warning for testing
    enabled: !!(user && student), // Only enable when fully authenticated
    onWarning: () => {
  ...
  // Update showInactivityWarning when warning state changes
  useEffect(() => {
    setShowInactivityWarning(showWarning);
  }, [showWarning]);

  const handleStayLoggedIn = () => {
    setShowInactivityWarning(false);
    resetTimer(); // This will reset the pause state and restart the timer
  };

  // Auto-redirect to dashboard on login (will be updated after showToast is defined)
  ...
  // ... Persistence helpers with better error handling
  const saveProgress = useCallback((subjects: Subject[]) => {
    try {
        ...
        const success = safeLocalStorageSet('study_ai_progress', progressMap);
        if (!success) {
  ...
  const getSavedProgress = useCallback(() => {
      return safeLocalStorageGet('study_ai_progress', {});
  }, []);

  // ... Profile update handlers with better error handling
  const handleUpdatePreferences = useCallback((newPrefs: UserPreferences) => {
     if (!student) return;
     const updatedStudent = { ...student, preferences: newPrefs };
     setStudent(updatedStudent);
     
     try {
         const db = safeLocalStorageGet('study_ai_users_db', {});
         if (db[student.rollNumber]) {
            db[student.rollNumber].profile = updatedStudent;
            safeLocalStorageSet('study_ai_users_db', db);
         }
     } catch(e) {
  ...
  const handleUpdateProfile = useCallback((newProfile: StudentProfile) => {
  ...
  // Optimized effects with better error handling
  useEffect(() => {
    if (typeof window !== 'undefined') {
      safeLocalStorageSet('study_ai_last_category', category);
    }
  }, [category]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      if (student) {
        safeLocalStorageSet('study_ai_student_profile', student);
      } else {
        try {
          localStorage.removeItem('study_ai_student_profile');
        } catch (e) {
          console.warn('Failed to remove student profile:', e);
        }
      }
    }
  }, [student]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      safeLocalStorageSet('study_ai_sessions', sessions);
    }
  }, [sessions]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      safeLocalStorageSet('study_ai_schedule', studySchedule);
    }
  }, [studySchedule]);

  useEffect(() => {
    if (typeof window !== 'undefined') {
      safeLocalStorageSet('study_ai_current_session_id', currentSessionId);
    }
  }, [currentSessionId]);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
        safeLocalStorageSet('study_ai_view', view);
    }
  }, [view]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        safeLocalStorageSet('study_ai_selected_subject', selectedSubjectId);
    }
  }, [selectedSubjectId]);
  ...
  // Memoize curriculum processing to prevent unnecessary recalculations
  ...
  }, [student?.department, category, selectedSemester, customSyllabus, student?.memory?.historicalWeakTopics]);

  // Update curriculum when processed data changes
  useEffect(() => {
  ...
  // Create the final handleLoginSuccess with toast after showToast is available
  const handleLoginSuccessWithToast = useCallback(() => {
    setView('dashboard');
    setSelectedSubjectId('');
    showToast('Welcome! Redirected to Dashboard');
  }, []);
  ...
  // Optimized message sending with Firebase integration
  const handleSendMessage = useCallback(async (e?: React.FormEvent, customInput?: string, activeMode: 'CONCEPT' | 'EXAM' = 'CONCEPT') => {
    e?.preventDefault();
    ...
    try {
      ...
      // Smart status updates based on AI response
      ...
      } else if (ltext.includes("excellent") || ltext.includes("correct") || ltext.includes("well done")) {
         if (currentSession?.topicId) {
  ...
  }, [input, isLoading, sessions, currentSessionId, selectedSubjectId, curriculum, student, category, style, selectedSemester]);       
  ...
  // Load AI history from Firebase on user authentication
  useEffect(() => {
    if (user && userData) {
      const loadAIHistory = async () => {
        try {
          const history = await getAIHistory();
          // You can use this history to populate previous conversations if needed
          console.log('Loaded AI history:', history.length, 'entries');
        } catch (error) {
  ...
  if (!user) return <Login onLogin={(profile) => setStudent(profile)} onLoginSuccess={handleLoginSuccessWithToast} />;
  ...
  if (!student) {
    // User is authenticated but no student profile - this shouldn't happen with Firebase
    return <Login onLogin={(profile) => setStudent(profile)} onLoginSuccess={handleLoginSuccessWithToast} />;
  }
  ...
  return (
    <ErrorBoundary>
      <div className={`flex h-screen bg-slate-50 dark:bg-graphite-base text-slate-800 dark:text-graphite-text-main overflow-hidden ${textSizeClass}`}>
        ...
        {/* Debug Panel (commented out - can be re-enabled for testing) */}
        {/* <InactivityDebugger
          isEnabled={!!(user && student)}
